...
const Register = () => {
  ...
  // Google response handler with useCallback to prevent re-initialization
  const handleGoogleResponse = useCallback(async (response) => {
    console.log('🔍 Google response received in Register page');
    try {
      ...
      if (result.success) {
        ...
        if (role === "Admin") {
          ...
          console.log('🔄 Redirecting to /dashboard/tailor');
  ...
  useEffect(() => {
    const initializeGoogleSignIn = () => {
      if (window.google && window.google.accounts) {
        ...
        // Render the button as a fallback
        renderGoogleButton();
      }
    };
  ...
  }, [handleGoogleResponse]); // Add dependency
      ...
      try {
        window.google.accounts.id.prompt((notification) => {
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            console.warn('⚠️ Google prompt not displayed or skipped');
            renderGoogleButton();
          }
        });
      } catch (error) {
